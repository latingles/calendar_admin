...
.merge-date-btn {
  ...
  padding: 10px 22px; font-size: 1.07rem; font-weight: 500; margin-top: 4px;
  cursor: pointer;
...
.merge-checkbox-label {
  display: flex; align-items: center; gap: 9px; font-size: 1rem; color: #232323; margin-top: 7px;
}
.merge-checkbox-label input[type="checkbox"] {
  width: 19px; height: 19px; accent-color: #fe2e0c; margin-right: 2px;
}
.merge-cohort-btn {
  width: 100%; background: #fe2e0c; color: #fff; border: none; font-weight: bold;
  font-size: 1.09rem; border-radius: 9px; padding: 14px 0; margin-top: 18px;
  cursor: pointer; box-shadow: 0 3px 13px 0 rgba(254,46,12,.07);
  letter-spacing: .5px;
}
@media (max-width: 600px) {
  .merge-row { flex-direction: column; gap: 10px; }
  .merge-col { min-width: 0; }
  .merge-dropdown-list { width: 98vw; left: 50%; transform: translateX(-50%); }
}
...
.merge-calendar-modal {
  ...
  width: 340px;
  max-width: 96vw;
  margin: 5vh auto; padding: 0 0 24px 0;
...
.merge-calendar-header {
  ...
  font-size: 1.21rem; font-weight: bold; padding: 24px 24px 6px 24px;
}
.merge-calendar-header button {
  background: #fff; border: 2px solid #ececec; border-radius: 9px;
  padding: 4px 14px; font-size: 1.4rem; color: #232323; cursor: pointer;
  font-weight: 600; transition: border .15s, background .13s;
}
.merge-calendar-header button:hover { border: 2px solid #fe2e0c; background: #fff7f4;}
#mergeCalendarMonth {
  font-size: 1.19rem; font-weight: 600; color: #232323;
  letter-spacing: .5px; flex: 1; text-align: center;
}
.merge-calendar-days {
  display: grid; grid-template-columns: repeat(7,1fr); gap: 2px;
  padding: 0 22px 0 22px; margin: 6px 0 7px 0;
  font-size: 1.08rem;
}
.merge-calendar-day-header {
  color: #b2b2b2; font-weight: 600; padding: 8px 0 5px 0; background: none;
  text-align: center;
}
.merge-calendar-day, .merge-calendar-day-inactive {
  padding: 12px 0;
  border-radius: 7px;
  cursor: pointer;
  font-size: 1.09rem;
...
.merge-calendar-done-btn {
  width: 92%; margin: 16px 4% 0 4%; background: linear-gradient(90deg,#fe2e0c 40%,#ff3e10 100%);
  color: #fff; font-size: 1.13rem; font-weight: bold; border-radius: 10px; border: 2px solid #232323;
  padding: 13px 0; cursor: pointer; transition: background .12s, border .11s;

...
$(function(){
  ...
  // Bring clicked event to top ONLY (no resize / no lane change)
  let zSeed = 5000;
  ...
  function renderWeek(resetScroll=false){
    ...
    // ---- Google Calendar–style overlap: right-most is front; equal widths ----
    const MAX_LEFT = 4 + (STACK_CAP - 1) * STACK_OFFSET; // largest left offset
    ...
    perDay.forEach((list,di)=>{
      ...
      list.forEach(ev=>{
        ...
        ev.stackIndex = Math.min(active.length, STACK_CAP-1); // 0..STACK_CAP-1
        active.push(ev);

        const top=(ev.start-START_H*60)*PX_PER_MIN;
        const h=(ev.end-ev.start)*PX_PER_MIN - 4;

        // lane 0 sits right-most; others step left; ALL share same width
        const leftPx = MAX_LEFT - ev.stackIndex * STACK_OFFSET;
        ...
        const $ev=$(`<div class="event ${ev.color||'e-blue'}" data-start="${ev.start}" data-end="${ev.end}">
            ...
            height: h+'px',
            left: leftPx+'px',
            width: `calc(100% - ${MAX_LEFT + 8}px)` // fixed width -> left slivers of behind cards stay visible
          });

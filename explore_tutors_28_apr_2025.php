...
<div class="noSelect">
  ...
  <style>
    ...
    .filter-button .icon { font-size: 14px; color: var(--text-muted); }
    .filters_dropdown {
      position: absolute;
      top: calc(100% + 4px);
      left: 0;
    ...
    .filters_dropdown.open { display: block; }
    .search-wrapper { position: relative; margin: 12px; }
    ...
    .filters_dropdown ul { list-style: none; margin: 0; padding: 0; }
    .filters_dropdown li.section-label {
      padding: 8px 12px;
      font-size: 14px;
      font-weight: 600;
      color: var(--text-section);
    }
    .filters_dropdown li.item {
      display: flex;
      align-items: center;
      justify-content: space-between;
      padding: 10px 12px;
    ...
    .filters_dropdown li.item:hover { background: var(--bg-hover); }
    .filters_dropdown li.item + li.item { border-top: 1px solid #f2f4f7; }
    .checkbox {
    ...
    .time-option.selected { background: var(--accent); color: #fff; border-color: var(--accent); }
    ...
    .day-option.selected { background: var(--accent); color: #fff; border-color: var(--accent); }
  </style>
  <div class="filters">
    <!-- 1: Class taught in -->
    <div class="filter">
      <div class="filter-button"><span class="label">Class taught in</span><div class="selection"><span class="value">English &amp; Spanish</span><span class="icon">▾</span></div></div>
      <div class="filters_dropdown">
        <div class="search-wrapper"><input placeholder="Type to search..." /><span class="search-icon">🔍</span></div>
        <ul><li class="section-label">Popular</li><li class="item"><span>English &amp; Spanish</span><span class="checkbox selected"></span></li><li class="item"><span>English (only)</span><span class="checkbox"></span></li><li class="item"><span>Spanish (only)</span><span class="checkbox"></span></li></ul>
      </div>
    </div>
    <!-- 2: English Level -->
    <div class="filter">
      <div class="filter-button"><span class="label">English Level</span><div class="selection"><span class="value">Basic</span><span class="icon">▾</span></div></div>
      <div class="filters_dropdown"><ul><li class="item">Basic</li><li class="item">Beginner</li><li class="item">Elementary</li><li class="item">Intermediate</li><li class="item">Upper Intermediate</li><li class="item">Advanced</li></ul></div>
    </div>
    <!-- 3: Availability -->
    <div class="filter">
      <div class="filter-button"><span class="label">I'm available</span><div class="selection"><span class="value">Anytime</span><span class="icon">▾</span></div></div>
      <div class="filters_dropdown">
        <ul>
          <li class="section-label">Times</li>
          <div class="segment-group"><div class="segment-label">Daytime</div><div class="segment-options"><div class="time-option"><span>🌅</span><span>9–12</span></div><div class="time-option"><span>☀️</span><span>12–15</span></div><div class="time-option"><span>🌤️</span><span>15–18</span></div></div></div>
          <div class="segment-group"><div class="segment-label">Evening & night</div><div class="segment-options"><div class="time-option"><span>🌇</span><span>18–21</span></div><div class="time-option"><span>🌙</span><span>21–24</span></div><div class="time-option"><span>🌑</span><span>0–3</span></div></div></div>
          <div class="segment-group"><div class="segment-label">Morning</div><div class="segment-options"><div class="time-option"><span>🌙</span><span>3–6</span></div><div class="time-option"><span>🌅</span><span>6–9</span></div></div></div>
          <li class="section-label">Days</li>
          <div class="day-options"><div class="day-option">Sun</div><div class="day-option">Mon</div><div class="day-option">Tue</div><div class="day-option">Wed</div><div class="day-option">Thu</div><div class="day-option">Fri</div><div class="day-option">Sat</div></div>
        </ul>
      </div>
    </div>
    <!-- 4: Class Type -->
    <div class="filter">
      <div class="filter-button"><span class="label">Class Type</span><div class="selection"><span class="value">Theoretical & Conversational</span><span class="icon">▾</span></div></div>
      <div class="filters_dropdown">
        <ul>
          <li class="section-label">Popular</li>
          <li class="item"><span>Theoretical & Conversational</span><span class="checkbox selected"></span></li>
          <li class="item"><span>Conversational (only)</span><span class="checkbox"></span></li>
        </ul>
      </div>
    </div>
    <!-- 5: Price per Month -->
    <div class="filter">
      <div class="filter-button"><span class="label">Price per Month</span><div class="selection"><span class="value">40 to 70</span><span class="icon">▾</span></div></div>
      <div class="filters_dropdown"><ul><li class="item">0–30</li><li class="item">30–50</li><li class="item">50–70</li><li class="item">70+</li></ul></div>
    </div>
  </div>
...
<script>
    $(function() {
      $('.filter-button').on('click', function(e) {
        e.stopPropagation();
        var $btn = $(this);
        var $dd = $btn.next('.filters_dropdown');
        var open = $dd.hasClass('open');
        $('.filters_dropdown').removeClass('open');
        $('.filter-button').removeClass('open');
        if (!open) { $dd.addClass('open'); $btn.addClass('open'); }
      });
      ...
      $('.filters_dropdown li.item').on('click', function(e) {        
        e.stopPropagation();
        var $item = $(this);
        var $cb = $item.find('.checkbox');
        if ($cb.length) { $cb.toggleClass('selected'); }
        else {
          var txt = $item.text().trim();
          $item.closest('.filter').find('.filter-button .value').text(txt);
          $item.closest('.filters_dropdown').removeClass('open');
          $item.closest('.filter').find('.filter-button').removeClass('open');
        }

...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* Now line with arrowheads */
  .now{position:absolute;left:0;right:0;height:3px;background:#ff3b30;z-index:3;pointer-events:none}
  .now::before{content:"";position:absolute;top:50%;left:2px;transform:translateY(-50%);border-top:6px solid transparent;border-bottom:6px solid transparent;border-right:10px solid #ff3b30}
  .now::after{content:"";position:absolute;top:50%;right:2px;transform:translateY(-50%);border-top:6px solid transparent;border-bottom:6px solid transparent;border-left:10px solid #ff3b30}
...
/* Compute Monday of a given date */
...
/* Example events (day = 0..6 from Monday) */
const events=[
  { day:2,title:"Conversation1",start:"07:00",end:"08:00",color:"e-blue",repeat:true },
  { day:2,title:"Conversation2",start:"07:00",end:"08:00",color:"e-blue",repeat:true },
  { day:2,title:"Conversation3",start:"07:00",end:"08:00",color:"e-blue",repeat:true },
...
$(function(){
  ...
  // Delegated click for overlap reordering
  let zSeed=1000;
  ...
  // Redraw "now" line every minute (it respects currentWeekStart)
  ...
  function renderWeek(){
    // Header
    const $head=$('#head');
    $head.find('.day-h').remove(); // keep the sticky gutter
    for(let i=0;i<7;i++){
    ...
    // Days grid
    const $grid=$('#grid');
    $grid.find('.day').remove(); // keep the gutter column
    const rows=(END_H-START_H)*(60/SLOT_MIN);
    const dayEls=[];
    ...
    perDay.forEach((list,di)=>{
      ...
      list.forEach(ev=>{
        ...
        const $ev=$(`<div class="event ${ev.color}" data-start="${ev.start}" data-end="${ev.end}">
            <div class="ev-top">
  ...
  function prep(e){return {...e,start:minutes(e.start),end:minutes(e.end)};}
});
</script>

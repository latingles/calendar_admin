...
<head>
  ...
  <style>
    ...
    .calendar_admin_details_create_cohort_shortname_list li {
      padding: 12px 18px; border-radius: 8px; font-size: 1rem; cursor: pointer; transition: background 0.18s;
    }
    .calendar_admin_details_create_cohort_dropdown_list li:hover,
    .calendar_admin_details_create_cohort_shortname_list li:hover {background: #f7f7f7; color: #fe2e0c;}
    .calendar_admin_details_create_cohort_dropdown_list button {
      display: block; width: 100%; margin-bottom: 13px;
      padding: 11px; background-color: #fe2e0c; color: white;
      border: none; border-radius: 8px; font-weight: bold; font-size: 1.01rem; cursor: pointer; margin-top: -3px;
    }
    .calendar_admin_details_create_cohort_dropdown_list strong {
      display: block; margin-bottom: 12px; font-weight: 700; font-size: 1.09rem; color: #232323;
    }
    .calendar_admin_details_create_cohort_teacher_list ul { list-style: none; padding: 0; margin: 0; }
    ...
    .calendar_admin_details_create_cohort_class_list ul { list-style: none; padding: 0; margin: 0; }
    .calendar_admin_details_create_cohort_class_list li {
      padding: 12px 18px; border-radius: 8px; font-size: 1rem; cursor: pointer; transition: background 0.18s;
    }
    .calendar_admin_details_create_cohort_class_list li:hover {background: #f7f7f7; color: #fe2e0c;}
    .calendar_admin_details_create_cohort_time_btn {
      padding: 10px 22px; border-radius: 26px; background: #fff; border: 2px solid #232323;
      font-size: 1.09rem; font-weight: 600; cursor: pointer; margin: 6px 7px 6px 0;
      display: inline-flex; align-items: center; position: relative; min-width: 112px;
    ...
    .calendar_admin_details_create_cohort_time_modal_backdrop {
      display: none; position: fixed; z-index: 2000; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.12);
    }
    .calendar_admin_details_create_cohort_time_modal {
      position: absolute; left: 50%; transform: translateX(-50%); top: 140px;
      background: #fff; border-radius: 13px; box-shadow: 0 12px 36px 0 rgba(0,0,0,.16);
      padding: 12px 0; min-width: 185px; z-index: 3000; max-height: 350px; overflow-y: auto; border: 1.5px solid #ededed;
    }
    .calendar_admin_details_create_cohort_time_modal ul {list-style: none; padding: 0; margin: 0;}
    .calendar_admin_details_create_cohort_time_modal li {
      font-size: 1.14rem; font-weight: 500; color: #232323; padding: 12px 30px; cursor: pointer; border-radius: 8px;
      text-align: left; transition: background 0.14s;
    }
    .calendar_admin_details_create_cohort_time_modal li:hover {background: #f7f7f7; color: #fe2e0c;}
...
<body>
  ...
  <div id="calendar_admin_details_create_cohort_modal_backdrop">
    <div id="calendar_admin_details_create_cohort_modal">
      ...
      <div id="conferenceTabContent" style="display:none;">
        ...
        <div class="conference_modal_repeat_row">
          ...
          <div style="flex:1;">
            <span class="conference_modal_label" style="font-weight:400;">Start On</span>
            <button class="conference_modal_date_btn">Select Date</button>
          </div>
        </div>
        <div style="display:flex; gap:12px; align-items:center; margin-bottom:7px;">
          <button class="conference_modal_date_btn" style="margin-right:0;">Start Time</button>
          <span>-</span>
          <button class="conference_modal_date_btn">End Time</button>
          <a class="conference_modal_findtime_link" href="#">Find a time</a>
          <span class="conference_modal_findtime_circle"></span>
        </div>
    ...
    <div class="calendar_admin_details_create_cohort_calendar_modal_backdrop" id="calendarDateModalBackdrop" style="display:none;">
      <div class="calendar_admin_details_create_cohort_calendar_modal" id="calendarDateModal">
        ...
        <div class="calendar_admin_details_create_cohort_calendar_days">
          <!-- Days grid will be rendered here -->
        </div>
    ...
    $(document).ready(function () {
      ...
      $('.calendar_admin_details_create_cohort_tab').click(function () {
        ...
        if(tab === 'conference'){
          ...
          $('#mainModalContent > :not(#conferenceTabContent)').show();
        }
      });
      ...
      $('#teacher1DropdownList li').click(function(){
        $('#teacher1DropdownBtn').contents().first()[0].textContent = $(this).text() + " ";
      ...
      $('#teacher2DropdownList li').click(function(){
        $('#teacher2DropdownBtn').contents().first()[0].textContent = $(this).text() + " ";
      ...
      $('#conferenceTeachersDropdownList li').click(function(){
        $('#conferenceTeachersDropdown').contents().first()[0].textContent = $(this).text() + " ";
        $('#conferenceTeachersDropdownList').hide();
      });
      ...
      $('.calendar_admin_details_create_cohort_time_btn').click(function(e){
        e.stopPropagation();
        $('#timeModalBackdrop').data('targetBtn', $(this).attr('id'));
        var btn = $(this);
        var offset = btn.offset();
        $('#timeModal').css({
          top: offset.top + btn.outerHeight() + 7,
          left: offset.left + btn.outerWidth()/2 - $('#timeModal').outerWidth()/2
        });
        $('#timeModalBackdrop').show();
      });
      $('#timeModalBackdrop li').click(function(){
        let btnId = $('#timeModalBackdrop').data('targetBtn');
        $('#' + btnId).text($(this).text()).addClass('selected');
        $('#timeModalBackdrop').hide();
      });
      $('#timeModalBackdrop').click(function(e){
      ...
      $(document).on('click', '.conference_modal_date_btn', function(e){
        ...
        // Default to Jan 2025 for Conference tab, else today
  ...
  <?php require_once('calendar_admin_details_cohorts_select_date.php');?>

...
// --- Demo Events (hour and min decide slot, 15-min granularity) ---
var demoEvents = [
  
  { date: '2025-08-09', day: 0, hour: 2, min: 0, duration:   0, title: "FL1", className: "calendar_admin_event_student", subtext: "2:00 - 2:10 AM", repeat: true },
  { date: '2025-08-09', day: 1, hour: 3,  min: 15, duration: 25, title: "Jonas", className: "calendar_admin_event_first",   subtext: "3:15 - 3:40 AM",  repeat: true },
  { date: '2025-08-09', day: 2, hour: 6,  min: 30, duration: 60, name: 'Mary Janes', avatar: 'https://randomuser.me/api/portraits/women/44.jpg', className: "calendar_admin_event_student", subtext: "6:30 - 7:30 AM", repeat: false, imgRepeat: true },
  { date: '2025-08-09', day: 2, hour: 1,  min: 0,  duration: 60, title: "Conversation", className: "calendar_admin_event_conversation", subtext: "1:00 - 2:00 AM", repeat: true },
  { date: '2025-08-09', day: 2, hour: 5,  min: 45, duration: 45, title: "Peer Talk",   className: "calendar_admin_event_peer",    subtext: "5:45 - 6:30 AM",  repeat: true },
  { date: '2025-08-09', day: 3, hour: 3,  min: 0,  duration: 60, title: "Team Meeting", className: "calendar_admin_event_team",   subtext: "3:00 - 4:00 AM" },
  { date: '2025-08-09', day: 4, hour: 1,  min: 30, duration: 60, title: "FL4",         className: "calendar_admin_event_fl",      subtext: "1:30 - 2:30 AM",  repeat: true },
  { date: '2025-08-09', day: 5, hour: 5,  min: 15, duration: 60, name: 'Mary Janes',   avatar: 'https://randomuser.me/api/portraits/women/43.jpg', className: "calendar_admin_event_student", subtext: "5:15 - 6:15 AM", repeat: true },
  { date: '2025-08-09', day: 6, hour: 2,  min: 45, duration: 60, title: "Busy",        className: "calendar_admin_event_busy",    subtext: "2:45 - 3:45 AM" }
];

// --- Render Time Labels (start at 1:00 AM, skip midnight) ---
function renderTimeLabels() {
  var container = document.getElementById('calendar_admin_time_labels_col');
  container.innerHTML = '';
  for (var slot = 4; slot < 96; slot++) {
    ...
    div.style.fontWeight = (min === 0) ? "600" : "400";
    div.style.color = (min === 0) ? "#111" : "#bbb";
    div.textContent = (min === 0 || min === 30)
      ? pad(hour) + ":" + pad(min)
      : "";
...
function renderCalendarBody(weekStart) {
  ...
  eventsThisWeek.forEach(function(ev) {
    var slotIdx = ev.hour * 4 + Math.floor(ev.min / 15); // 0=00:00, 4=1:00AM, 8=2:00AM etc
    var renderRow = slotIdx - 4; // adjust so 1:00AM aligns to first row (row 0)
    var slots = Math.ceil(ev.duration / 15) || 1;
    // Prevent overflow and only render if in bounds
    if (renderRow >= 0 && renderRow < 92) {
      var free = true;
      for (var i = 0; i < slots; i++) if (grid[renderRow + i] && grid[renderRow + i][ev.day]) free = false;
      if (!free) return;
      grid[renderRow][ev.day] = { ev: ev, rowspan: slots };
      for (var i = 1; i < slots; i++) grid[renderRow + i][ev.day] = { skip: true };
    }
  });
  for (var r = 0; r < 92; r++) {
...
function updateRedBar() {
  ...
  var rowHeight = 23;
  var slotIdx = hours * 4 + Math.floor(mins / 15);
  var slotOffset = ((mins % 15) / 15) * rowHeight;
  // If before 1:00 AM, do not show
  if (slotIdx < 4) {
    document.getElementById('calendar_admin_red_bar').style.display = "none";
    return;
  }
  var cellTop = (slotIdx - 4) * rowHeight + slotOffset;
  if(slotIdx > 95) cellTop = (96 - 4) * rowHeight;

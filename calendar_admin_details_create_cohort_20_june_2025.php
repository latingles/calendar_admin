  ...
  <script>
    $(document).ready(function () {
      ...
      $('.calendar_admin_details_create_cohort_tab').click(function () {
        ...
        // Hide both if not cohort/conference
        if(tab !== "cohort" && tab !== "conference"){
          $('#mainModalContent').hide();
          $('#conferenceTabContent').hide();
        }
      });

      // Dropdowns (cohort/class/teacher/shortname, conference dropdowns)
      ...
      // ===== CALENDAR PICKER LOGIC (unchanged) =====
      ...
      // Teachers (dropdown text)
      let teachers = $('#conferenceTabContent #conferenceTeachersDropdown').text().replace(/\s+/g, ' ').trim();

      // Attendees list (all .conference_modal_attendee)
      let attendees = [];
      $('#conferenceTabContent .conference_modal_attendee').each(function() {
        // If it has a .conference_modal_attendee_name (cohort) use that, else use the email (second span)
